...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if ($_FILES['profile']['size'] <= 2000000) {
                    ...
                    if (in_array($imageFileType, ['jpg', 'jpeg', 'png'])) {
                        ...
                        if (move_uploaded_file($_FILES['profile']['tmp_name'], $targetFile)) {
                            $loginStatus = "The file " . $_POST['username'] . " has been uploaded.";
                        } else {
                            $loginStatus = "Error: Unable to upload your file.";
                        }
                    } else {
                        $loginStatus = "Error: Only JPG, JPEG, and PNG files are allowed.";
                    }
                } else {
                    $loginStatus = "Error: File size is too large.";
                }
            } else {
                $loginStatus = "Error: File is not a valid image.";
            }
        } else {
            $loginStatus = "No file uploaded.";
        }
        ...
        try {
            ...
            if ($stmt->execute()) {
                header('location: workshop-8-2.php?username=' . $_POST["username"]);
            } else {
                echo "<div class='alert alert-danger'>Error: Unable to insert user data.</div>";
            }
